...
「AIが組んだ体系を人間が目視確認してから先へ進む」というChatCarloの
中核思想を、ジオメトリー確認だけでなく計算パイプライン全体に広げる。
3つの関門を作る:

1. **ジオメトリー確認** — 既存の `chatcarlo preview`（実装済み、変更不要）
2. **光子軌跡確認** — 小historyで軌跡を3D表示する `chatcarlo trace`（新規）
3. **結果確認** — 線量/H*(10)マップを断面図にする `chatcarlo plot`（新規）
...
## Deliverable 1: `chatcarlo trace` — 光子軌跡の3D可視化
...
### 実装 1-a: 軌跡レコーダ（chatcarlo/transport.py）
...
- 飛行区間の始点 `o`・終点 `o + d*ds[:,None]`・エネルギー `e` は
  [transport.py:256-271](../chatcarlo/transport.py) で計算済み。
...
### 実装 1-b: HTMLへの重ね描き（chatcarlo/preview.py）
...
### 実装 1-c: CLI（chatcarlo/__main__.py）
...
## Deliverable 2: `chatcarlo plot` — 線量マップの断面図

### CLI仕様

```bash
python -m chatcarlo plot dose.npz [-o dose_maps.png] [--scene scene.yaml]
...
### 実装（chatcarlo/plotting.py 新規 + __main__.pyにcmd_plot）
...
---
name: vive-check
description: ChatCarloのシーンを人間と確認しながら段階実行する
...
## 関門1: ジオメトリー確認
1. `.venv/bin/python -m chatcarlo validate <scene>` — エラーなら修正提案して停止
2. `.venv/bin/python -m chatcarlo preview <scene> -o <scene名>_preview.html`
3. `open <html>` でブラウザ表示し、AskUserQuestionで確認:
   「ジオメトリーは意図通りですか？」（はい / 修正が必要）
   修正指示があればscene.yamlを編集して関門1を再実行

## 関門2: ビーム・軌跡確認
1. `.venv/bin/python -m chatcarlo trace <scene> -n 200 --seed 42 -o ..._trace.html`
...
## 関門4: 結果確認
1. `.venv/bin/python -m chatcarlo plot <npz> --scene <scene> -o ..._maps.png`
...
- [ ] `chatcarlo trace examples/chest_room.yaml -n 200` が10秒以内に
      完了し、ブラウザで軌跡・マーカー・凡例が表示される
- [ ] 同HTMLで軌跡チェックボックスをオフにすると既存previewと同等の表示
- [ ] `chatcarlo run ... --dose-out d.npz` → `chatcarlo plot d.npz --scene ...`
