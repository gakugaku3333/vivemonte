...
!                     ****************
!
!  ChatCarlo cross-check: K-shell fluorescence, W (tungsten) slab, 100 keV
!  pencil beam, FLUORESCENCE OFF (IEDGFL=0).
!
...
!              escape a 0.035 cm W slab - their energy is deposited
!              locally regardless via the EGS5 kinetic-energy cutoff)
!    Density: PEGS5 default density for element W (tungsten) is used
!              as-is (no RHO override), matching xraylib's
!              ElementDensity(74)=19.3 g/cm3 - ChatCarlo has no density
!              override for W either (see egs5-operator.md).
!
...
!     geom passes info to our howfar routine
     ...
     *       uin,vin,win,wtin
      ...
      real*8 anorm,band5670                    ! Local variables
      integer i,j,ncase
...
! nmed and dunit default to 1, i.e. one medium and we work in cm
      ...
      chard(1) = 0.035d0     !  optional, but recommended to invoke
...
!     nreg : number of region
      ...
      med(3)=0
      med(2)=1
! Regions 1 and 3 are vacuum, region 2, W (tungsten)
...
! defined before hatch call
      ...
      write(6,130)
130   format(/' Start fluor_off_w'/' Call hatch to get cross-section',
     *' data')
...
!    Pick up cross section data for tungsten
...
!-----------------------------------------------------------------------
      zbound=0.035d0
!     Plate is 0.035 cm (350 micron) thick
...
!    Get average energies
     ...
     *i=1,4)
160   format(/' For',F6.1,' keV photons incident on',F6.3,'cm of W',
...
!  Observable B: fraction of escaped photons in the 56-70 keV
!  W K-alpha/K-beta fluorescence-peak band (K-alpha1/2 ~57.98/59.32 keV,
!  K-beta1/2 ~67.24/69.10 keV). bin j covers energy ((j-1)*bwidth,
!  j*bwidth] MeV, so bins 57-70 cover (0.056,0.070] MeV.
      band5670=0.0
      do j=57,70
        band5670=band5670+ebin(j)
      end do
      write(6,167) band5670, band5670/escn
167   format(' Escaped photons in 56-70 keV band (count)=',F12.1/
     *' FRACTION OF ESCAPED PHOTONS IN 56-70 keV BAND=',F10.6//)
...
!  We are in the W Slab plate - check the geometry
