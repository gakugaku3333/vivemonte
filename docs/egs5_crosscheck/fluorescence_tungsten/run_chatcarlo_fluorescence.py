"""K殻蛍光X線EGS5相互検証（タングステン）: ChatCarlo側の実行スクリプト。

幾何・線質・許容基準はPREREGISTRATION.md参照。100 keV単色鉛筆ビーム->
タングステンスラブ(厚さ0.035cm)、蛍光ON/OFFの2条件、n=1,000,000、seed=1。
transport_photonsを直接叩く。docs/egs5_crosscheck/fluorescence/の鉛版と
同一パターン（材料・厚さ・ピーク帯のみ変更）。

材料名は"W"（元素記号）を使う — "tungsten"はMATERIAL_ALIASESに
scene.yaml向けの短縮名として登録されていない（_XAAMDI_FILESのみに存在）ため、
scene.yamlのvalidate_sceneを経由しないこの直接呼び出しでは"W"を使う。
"""
...
def _slab_arrays(n, seed):
    geometry = Geometry([{
        "name": "slab", "shape": "box", "material": "W",
        "center": [0.0, 0.0, 0.0],
...
def run(fluorescence_enabled: bool) -> dict:
    ...
    return {
        ...
        "linear_mu_per_cm": float(linear_mu("W", ENERGY_KEV)[0]),
...
if __name__ == "__main__":
    ...
    with open("docs/egs5_crosscheck/fluorescence_tungsten/chatcarlo_results.json", "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print("\n書き出し: docs/egs5_crosscheck/fluorescence_tungsten/chatcarlo_results.json")
