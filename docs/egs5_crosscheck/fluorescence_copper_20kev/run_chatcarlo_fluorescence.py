"""K殻蛍光X線エネルギー依存性EGS5相互検証（銅、20 keV）: ChatCarlo側の実行スクリプト。

幾何・線質・許容基準はPREREGISTRATION.md参照。20 keV単色鉛筆ビーム->
銅スラブ(厚さ0.01cm)、蛍光ON/OFFの2条件、n=1,000,000、seed=1。
transport_photonsを直接叩く。docs/egs5_crosscheck/fluorescence_copper/(100 keV版)
...
def _slab_arrays(n, seed):
    ...
    # 追加で~50cmの空気中を飛行することになり、低エネルギーでは無視できない
    # 減弱を生む（water20kev検証で20keV・60cm空気路で約5.5%減弱と判明、
    # docs/lessons_learned.md参照）。EGS5のtutor5パターン（スラブ外は真空）
    # に合わせるため、bbox_marginと線源ギャップを意図的に極小にする
    # （100 keV版の`fluorescence_copper/`は既定値のままだが、100 keVでは
    # 空気減弱が約1%と小さく許容基準内に収まっていたため問題化しなかった）。
    geometry = Geometry([{
...
def run(fluorescence_enabled: bool) -> dict:
    ...
    # 標本標準偏差から直接計算（history毎の透過エネルギー、脱出しなければ0）
...
if __name__ == "__main__":
    ...
    with open("docs/egs5_crosscheck/fluorescence_copper_20kev/chatcarlo_results.json", "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print("\n書き出し: docs/egs5_crosscheck/fluorescence_copper_20kev/chatcarlo_results.json")
