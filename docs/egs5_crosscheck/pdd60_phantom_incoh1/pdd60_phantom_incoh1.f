...
!                     ***************************
!                     *                         *
!                     *  pdd60_phantom_incoh1   *
!                     *                         *
!                     ***************************
!
!  EGS5 user code for the viveMonte/EGS5 cross-check, Phase 2b
!  residual-check Step 1 (docs/egs5_crosscheck/plan_residual_check.md).
!  Identical in every respect to pdd60_phantom.f (Phase 2b, 47-bin
!  PDD/OCR tally) EXCEPT: (1) the companion PEGS5 input
!  (pdd60_phantom_incoh1.inp) sets INCOH=1 instead of INCOH=0, so the
!  PEGS5 data file carries S(q) incoherent-scattering-function tables
!  in addition to the IBOUND=1 bound-Compton profile data; and (2) this
!  user code sets incohr(2)=1 for the phantom region (region 2) so that
!  HATCH actually turns on S(q)-weighted (Waller-Hartree, ICPROF=0 i.e.
!  no Doppler broadening) incoherent scattering at runtime -- following
!  the tutor7.f sample (egs5/tutorcodes/tutor7/tutor7.f, line 132:
!  incohr(2)=1) which is EGS5's own worked example of INCOH=1 usage.
!  Without incohr(2)=1, EGS5 would silently fall back to free-electron
!  Klein-Nishina despite the PEGS5 data containing S(q) tables (HATCH's
!  consistency check in egs5_hatch.f only requires ibound=1 and
!  incoh=1 in the PEGS5 file when incohm(im)=1; it does not force
!  incohr on by itself).
!
!  Geometry/source are otherwise reused unchanged from Phase 2a's
