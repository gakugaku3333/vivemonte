...
!                     *******************
!
!  ChatCarlo cross-check: K-shell fluorescence, Cu slab, 100 keV pencil
!  beam, FLUORESCENCE ON (IEDGFL=1). Second material in the
...
!  to match ~3.1 mean free paths at 100 keV, same as the Pb run), and
...
! nmed and dunit default to 1, i.e. one medium and we work in cm
      ...
      chard(1) = 0.75d0      !  optional, but recommended to invoke
...
!     slab - required in ADDITION to INCOH=1 in the PEGS5 &INP namelist
      iedgfl(2)=1
!     FLUORESCENCE ON for this run (fluor_on_cu)
...
! defined before hatch call
      ...
      write(6,130)
130   format(/' Start fluor_on_cu'/' Call hatch to get cross-section',
     *' data')
...
!-----------------------------------------------------------------------
      zbound=0.75d0
!     Plate is 0.75 cm thick (~3.1 mean free paths at 100 keV, same as
!     the Pb run's mfp-count - see PREREGISTRATION.md)
