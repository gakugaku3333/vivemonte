...
!                     ****************
!
!  ChatCarlo cross-check: K-shell fluorescence, Pb slab, 150 keV pencil
!  beam, FLUORESCENCE OFF (IEDGFL=0).
!
!  Built from tutorcodes/tutor5 (LATCH-based primary/Compton/Rayleigh
!  classification) + tutorcodes/tutor7 (Pb slab, 150 keV, IEDGFL flag,
...
!     geom passes info to our howfar routine
      ...
      common/score/count(4),entot(4),esctot,escn,ebin(160),bwidth
...
! nmed and dunit default to 1, i.e. one medium and we work in cm
      ...
      chard(1) = 0.135d0      !  optional, but recommended to invoke
...
!     slab - required in ADDITION to INCOH=1 in the PEGS5 &INP namelist
      iedgfl(2)=0
!     FLUORESCENCE OFF for this run (fluor_off_pb150)
...
! defined before hatch call
      ...
      write(6,130)
130   format(/' Start fluor_off_pb150'/' Call hatch to get cross-',
     *'section data')
...
!-----------------------------------------------------------------------
      zbound=0.135d0
!     Plate is 0.135 cm thick (~3.09 mean free paths at 150 keV,
!     linear_mu(Pb,150keV)~22.87/cm - see PREREGISTRATION.md)
...
!    Total escaped-photon energy/count and spectrum (both directions)
          esctot = esctot + e(np)
          escn = escn + 1.
          ibin = min0(max0(int(e(np)/bwidth + 0.999),1),160)
