...
ボクセル吸収線量（`vivemonte/tally.py`）はタリー専用の一様グリッドとして
...
ことをテストで相互検証している（`tests/test_tally.py`）。線量は
`Gy/history`単位で出力し、mAs等への校正は次段の課題（フォトン数校正未実装）。
...
- [ ] 空間線量H*(10)タリー（ICRU球換算係数の適用が必要、カーマとは別途）※次段
