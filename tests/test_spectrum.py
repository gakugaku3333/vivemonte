...
from vivemonte import transport


def test_spekpy_available():
    assert transport._HAS_SPEKPY, "spekpyがインストールされていません（.venv/bin/pip install spekpy）"


def test_spectrum_normalized_and_bounded():
    e, w = transport._default_spectrum(kvp=120.0, filtration_mm_al=2.5, anode_angle_deg=12.0)
...
def test_higher_filtration_hardens_spectrum():
    ...
    e_thin, w_thin = transport._default_spectrum(kvp=100.0, filtration_mm_al=1.0, anode_angle_deg=12.0)
    e_thick, w_thick = transport._default_spectrum(kvp=100.0, filtration_mm_al=5.0, anode_angle_deg=12.0)
...
def test_sample_spectrum_draws_within_range(monkeypatch):
    ...
    energies = transport.sample_spectrum(src, 5000, rng)
...
def test_explicit_spectrum_overrides_spekpy():
    ...
    energies = transport.sample_spectrum(src, 100, rng)
    assert np.all(energies == 50.0)


def test_fallback_spectrum_used_when_spekpy_unavailable(monkeypatch):
    monkeypatch.setattr(transport, "_HAS_SPEKPY", False)
    e, w = transport._default_spectrum(kvp=100.0, filtration_mm_al=2.5, anode_angle_deg=12.0)
    assert np.isclose(w.sum(), 1.0)
    assert e.max() <= 100.0
